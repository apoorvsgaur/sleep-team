...
folder_path = "/Users/apoorvsgaur/Desktop/Apoorv/Classes/Spring 2016/EPICS/epics-sleep-video/23001_24M/23001_24M Sleep Videos/130903- Start of Night/*"
...
for video in list_of_videos:
		...
		while True:
			...
			if  second not in plot_dict.keys():
				plot_dict[second] =	no_white_pixels
				sleep_dict[second] = status
				print str(frame_count) + ": " + str(round(frame_count/16.02)) + " No. of white pixels:" + str(no_white_pixels)
			...
			print "Average number of white pixels in the past 2 minutes: " + str(average)
			if average > 75:
				text = "Awake"
				status = 1
			else:
...
f = open("No_White_Pixels_Video1.txt", "w")
f.write("Time    Activity  	Sleep Status\n")

#Calculating Activity Count and storing the results in a file
for seconds, no_of_white_pixels in plot_dict.items():
	contours_in_the_last_60_seconds += no_of_white_pixels
	if (seconds % 60 == 0):
...
pyplot.ylabel('No. of White Pixels')
