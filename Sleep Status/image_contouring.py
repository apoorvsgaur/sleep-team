...
from sleep_status import delegate_list_of_motion_activity


#given folder path, compile all paths of videos
folder_path = "/Users/apoorvsgaur/Desktop/Apoorv/Classes/Spring 2016/EPICS/epics-sleep-video/Sleep Night 2/*"
#def contouring_processing(folder_path):
list_of_videos =  glob.glob(folder_path)
list_of_videos_without_ebe = []

for video in list_of_videos:
	if ".ebe" not in video:
		list_of_videos_without_ebe.append(video)

print(str(list_of_videos_without_ebe))
...
for video in list_of_videos_without_ebe:
	#print video
	no_of_videos += 1
	print (no_of_videos)
	#if (no_of_videos == 2):
...
f = open("23055_9M_Fall_2016.txt", "w")
...
for seconds, no_of_contours in plot_dict.items():
	...
	if (seconds % 60 == 0):
		motion_activity.append(round(contours_in_the_last_60_seconds/60))
...
print(str(motion_activity))
delegate_list_of_motion_activity(motion_activity)
